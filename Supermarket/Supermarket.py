...
def admin_page_1():
    clear_window()
    global update_item_spinbox, vals
    vals = ["Apple", "Banana", "Carrot", "Potato", "Bread", "Cake"]
    update_item_label = Label(root, text="Item: ", background="#faf9ed")
    update_item_spinbox = Spinbox(root, values=vals, wrap=True, width=20)
    refill_complete_stock_button = Button(root, text="Refill complete stock", command=refill_complete_stock)
    update_item_button = Button(root, text="Continue", command=admin_page_2)
    update_item_label.place(x=220, y=30)
    update_item_spinbox.place(x=260, y=30)
    refill_complete_stock_button.place(x=200, y=65)
    update_item_button.place(x=340, y=65)


def admin_page_2():
    global update_stock_spinbox, item_val
    item_val = update_item_spinbox.get()
    if update_item_spinbox.get() == "" or title_case(update_item_spinbox.get()) not in vals:
        showerror("Update Stock Error", "Please select a valid item")
        return None

    quant_val = None
    c = supermarket_db.execute(f"SELECT QUANTITY FROM ITEMS WHERE ITEM = '{update_item_spinbox.get().upper()}'")
    for i in c:
        quant_val = i[0]

    clear_window()
    current_item_label = Label(root, text=f"{title_case(item_val)} x {quant_val}", background="#faf9ed")
    update_stock_label = Label(root, text="Stock: ", background="#faf9ed")
    update_stock_spinbox = Spinbox(root, from_=1, to=15, wrap=True, width=20)
    refill_item_stock_button = Button(root, text="Refill stock", command=refill_item_stock)
    update_stock_button = Button(root, text="Update stock", command=update_item_stock)
    current_item_label.place(x=280, y=10)
    update_stock_label.place(x=220, y=50)
    update_stock_spinbox.place(x=260, y=50)
    refill_item_stock_button.place(x=225, y=85)
    update_stock_button.place(x=325, y=85)


def update_item_stock():
    update_stock = update_stock_spinbox.get()
    if not update_stock.isnumeric():
        showerror("Update Stock Error", "Please ensure your stock is a number")
        return None

    elif 0 >= int(update_stock) > 15:
        showerror("Update Stock Error", "Please ensure your stock is between 1-15")
        return None

    supermarket_db.execute(f"UPDATE ITEMS "
                           f"SET QUANTITY = {update_stock} "
                           f"WHERE ITEM = '{item_val.upper()}'")
    supermarket_db.commit()
    admin_page_1()
    showinfo("Update Stock Successful", f"The item: {title_case(item_val)} has "
                                        f"successfully been updated to {update_stock}")


def refill_item_stock():
    supermarket_db.execute(f"UPDATE ITEMS SET QUANTITY = 15 WHERE ITEM = '{item_val.upper()}'")
    supermarket_db.commit()
    admin_page_1()
    showinfo("Update Stock Successful", f"The item: {title_case(item_val)} has "
                                        f"successfully been refilled")


def refill_complete_stock():
    supermarket_db.execute("UPDATE ITEMS SET QUANTITY = 15")
    supermarket_db.commit()
    login_page()
    showinfo("Update Stock Successful", "All items have successfully been fully refilled")
...
def get_pos():
    ...
    for i in pos:
        if not i[2]:
            ...
            return i[0], i[1]
        counter += 1
    showwarning("Display Error", "Too many items on screen")
...
def add_items_fruits():
    ...
    if quantity_spinbox_apple.get() != "":
        ...
        elif not checkout_item_stock("apple", quantity_spinbox_apple.get()):
            return None
    ...
    if quantity_spinbox_banana.get() != "":
        ...
        elif not checkout_item_stock("banana", quantity_spinbox_banana.get()):
            return None
...
def add_items_vegetables():
    ...
    if quantity_spinbox_carrot.get() != "":
        ...
        elif not checkout_item_stock("carrot", quantity_spinbox_carrot.get()):
            return None
    ...
    if quantity_spinbox_potato.get() != "":
        ...
        elif not checkout_item_stock("potato", quantity_spinbox_potato.get()):
            return None
...
def add_items_bakery():
    ...
    if quantity_spinbox_bread.get() != "":
        ...
        elif not checkout_item_stock("bread", quantity_spinbox_bread.get()):
            return None
    ...
    if quantity_spinbox_cake.get() != "":
        ...
        elif not checkout_item_stock("cake", quantity_spinbox_cake.get()):
            return None
...
def edit_item_check():
    global edit_item_quantity
    c = supermarket_db.execute(f"SELECT QUANTITY FROM {table} WHERE ITEM = '{edit_item_spinbox.get()}'")
    edit_item_quantity = None
    for i in c:
        edit_item_quantity = i[0]
...
def get_quantity(table_, item):
    c = supermarket_db.execute(f"SELECT ITEM, QUANTITY FROM {table_}")
    ret = None
    for i in c:
        if i[0] == item:
            ret = i[1]
...
def delete_item():
    item_quant = get_quantity("ITEMS", delete_item_val.upper())
    my_quant = get_quantity(table, delete_item_val)
    ...
    supermarket_db.execute(f"UPDATE ITEMS SET QUANTITY = {item_quant + my_quant} WHERE "
                           f"ITEM = '{delete_item_val.upper()}'")
    supermarket_db.commit()
    show_basket()
...
def edit_item_page_2():
    ...
    clear_window()
    back_button()
    editing_item_label = Label(root, text=f"Edit: {edit_item} x {edit_item_quantity}", background="#faf9ed")
    edit_quantity_label = Label(root, text="Quantity: ", background="#faf9ed")
    edit_quantity_spinbox = Spinbox(root, from_=1, to=15, wrap=True)
    find_edit_item_button = Button(root, text="Update", command=edit_item_do)
...
def edit_item_do():
    ...
    if int(edit_quantity_spinbox.get()) > edit_item_quantity:
        if int(edit_quantity_spinbox.get()) <= get_quantity("ITEMS", edit_item.upper()):
            supermarket_db.execute(
                f"UPDATE ITEMS SET QUANTITY = {get_quantity('ITEMS', edit_item.upper()) - (int(edit_quantity_spinbox.get()) - edit_item_quantity)} "
                f"WHERE ITEM = '{edit_item.upper()}'")

        else:
            showerror("Edit Item Error", "The edited item is on low stock. Please select a lower quantity")
            return None

    elif int(edit_quantity_spinbox.get()) < edit_item_quantity:
        supermarket_db.execute(
            f"UPDATE ITEMS SET QUANTITY "
            f"= {get_quantity('ITEMS', edit_item.upper()) + (edit_item_quantity - int(edit_quantity_spinbox.get()))} "
            f"WHERE ITEM = '{edit_item.upper()}'")
...
def add_search_item():
    if not checkout_item_stock(search_upper, quantity_spinbox_search.get()):
        return None
    search_check = False
    search_results = []
...
def checkout_item_stock(item, quantity):
    stock_from_db = supermarket_db.execute(f"SELECT QUANTITY FROM ITEMS WHERE ITEM = '{item.upper()}'")
    stock = None
    for i in stock_from_db:
        stock = i[0]
    if stock == 0:
        showerror("Checkout Error", f"The item {item.upper()} is out of stock.")
        return False

    elif int(quantity) > stock:
        showerror("Checkout Error", f"There is only {stock} of the item: {item.upper()} left")
        return False

    else:
        supermarket_db.execute(f"UPDATE ITEMS SET QUANTITY = {stock - int(quantity)} WHERE ITEM = '{item.upper()}'")
        supermarket_db.commit()
        return True


def checkout():
    supermarket_db.execute(f"DELETE FROM {table}")
    supermarket_db.commit()
    homepage()
    showinfo("Checkout Successful", "The items have successfully been checked out.")
...
pos = [[200, 100, False], [370, 100, False], [200, 270, False], [370, 270, False]]
login_page()
root.mainloop()
