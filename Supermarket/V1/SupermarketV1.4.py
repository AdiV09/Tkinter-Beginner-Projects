...
def homepage():
    ...
    global fruit_var, veg_var, bake_var
    category_label = Label(root, text="Categories", width=width, background="#faf9ed")
    category_menu = Menubutton(root, text="Categories")
    fruit_var = tk.StringVar()
    veg_var = tk.StringVar()
    bake_var = tk.StringVar()
    category_menu.menu = tk.Menu(category_menu, tearoff=0)
    category_menu["menu"] = category_menu.menu
    category_menu.menu.add_radiobutton(label="Fruits", variable=fruit_var, command=print_fruit)
    category_menu.menu.add_radiobutton(label="Vegetables", variable=veg_var, command=print_veg)
    category_menu.menu.add_radiobutton(label="Bakery", variable=bake_var, command=print_bake)
    featuring_label = Label(root, text="Featured today: ", background="#faf9ed")
    #
    # featuring_label.place(x=175, y=70)
    category_menu.place(x=10, y=150)
...
def show_fruits():
    ...
    quantity_spinbox_apple = Spinbox(root, from_=0, to=15, wrap=True, width=10)
    x, y = 200, 100
    ...
    quantity_spinbox_banana = Spinbox(root, from_=0, to=15, wrap=True, width=10)
    x = 370
...
def show_vegetables():
    ...
    quantity_spinbox_carrot = Spinbox(root, from_=0, to=15, wrap=True, width=10)
    x, y = 200, 100
    ...
    quantity_spinbox_potato = Spinbox(root, from_=0, to=15, wrap=True, width=10)
    x = 370
...
def show_bakery():
    ...
    quantity_spinbox_bread = Spinbox(root, from_=0, to=15, wrap=True, width=10)
    x, y = 200, 100
    ...
    quantity_spinbox_cake = Spinbox(root, from_=0, to=15, wrap=True, width=10)
    x = 370
...
def run_menu_check():
    print(fruit_var, veg_var, bake_var)
...
def show_basket():
    ...
    for i in cursor:
        ...
        item_button = Label(root, text=f"{i[1]} x {i[3]}        ${round(i[2], 2)}")
...
supermarket_db.commit()
supermarket_db.execute("UPDATE sqlite_sequence SET seq = 0")
supermarket_db.commit()
homepage()
root.mainloop()
