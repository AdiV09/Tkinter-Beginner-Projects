...
def checkout():
    basket = supermarket_db.execute(f"SELECT ITEM, QUANTITY FROM {table}")
    quant_basket = []
    for i in basket:
        quant_basket.append(i)
    if len(quant_basket) == 0:
        showerror("Checkout Error", "Please enter items in your basket before checking out.")
        return None
    basket = supermarket_db.execute(f"SELECT ITEM, QUANTITY FROM ITEMS")
    avail_basket = []
    for j in basket:
        avail_basket.append(j)
    for a in quant_basket:
        for b in avail_basket:
            if a[0].upper() == b[0]:
                if b[1] >= a[1]:
                    supermarket_db.execute(f"UPDATE ITEMS SET QUANTITY = {b[1] - a[1]} WHERE ITEM = '{a[0].upper()}'")

                elif b[1] < 1:
                    showerror("Checkout Error", f"The item {a[0].upper()} is out of stock.")
                    return None

                else:
                    showerror("Checkout Error", f"There is only {b[1]} of the item: {a[0].upper()} left")
                    return None
